...
@timetable_router.get("/by_group/{group_num}", response_model=list[Event])
...
@timetable_router.get("/by_room/{room_num}", response_model=list[Event])
async def get_timetable_by_room(room_num: str, start: datetime.date, end: datetime.date) -> list[Event]:
    logger.debug(f"Getting lessons for room {room_num}")
    room = await utils.get_list_rooms(db.session, room_num)
    return [Event.from_orm(row) for row in await utils.get_room_lessons_in_daterange(room, start, end)]


@timetable_router.get("/by_lecturer", response_model=list[list[Event]])
async def get_timetable_by_lecturer(first_name: str, middle_name: str, last_name: str, start: datetime.date, end: datetime.date) -> list[list[Event]]:
